...
first10year=function(a,b,c,d,e,an1){
  ...
  stream1=aggregate(tmp.1$streamflow,by=list(tmp.1$wy),FUN=sum)
  stream2=aggregate(tmp.2$streamflow,by=list(tmp.2$wy),FUN=sum)
  stream3=aggregate(tmp.3$streamflow,by=list(tmp.3$wy),FUN=sum)
  stream4=aggregate(tmp.4$streamflow,by=list(tmp.4$wy),FUN=sum)
  stream5=aggregate(tmp.5$streamflow,by=list(tmp.5$wy),FUN=sum)
...
stream=cbind(thin,thin.warm)
setwd("D:/arnas/out/Totales/")
write.table(stream,"10yearsafterthining_stream.csv",sep=";",dec=",",col.names = T,row.names = F)
